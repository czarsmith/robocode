...
public abstract class Segment {
    ...
    public Factor getBestFactor(Intel enemy) {
        ...
        if (matches(enemy, enemy.getTime())) {
    ...
    public void saveFactor(Intel enemy, GuessBullet b, double factor) {
        if (matches(RobotManager.getEnemy(b.getEnemy()), b.getStartTime())) {
            List<Factor> stats = getStats(enemy.getName());
            for (Factor stat : stats) {
